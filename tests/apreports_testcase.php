...
/*----------------------------------------------------------------------------*/        
        ...
        foreach($mdl_user_count as $u){
            ...
            $this->assertTrue(in_array($u->id, array(1,2,465,8251,9584,9541)), sprintf("userid %d not found in array of expected values", $u->id));
        }
        ...
        $this->assertEquals(2,count($gmember));
        
        
        //test group membership
        $members_01 = $DB->get_records('groups_members', array('groupid'=>666));
        $this->assertEquals(2,count($members_01));
        foreach($members_01 as $m){
            $this->assertTrue(in_array($m->userid, array(465, 8251)));
        }
